...
import water.godcoder.module.modules.chat.AutoEZ;
...
import water.godcoder.util.MathUtil;
...
public class AutoCrystal extends Module {
    ...
    private Setting<Boolean> autoOffhand = register(Settings.booleanBuilder("IDK2").withValue(false).withVisibility(b -> general.getValue()).build());
    private Setting<Boolean> LegCrystals = register(Settings.booleanBuilder("IDK1").withValue(false).withVisibility(b -> general.getValue()).build());
    private Setting<Boolean> AutoCrystalPVPInfo = register(Settings.booleanBuilder("Bug2").withValue(false).withVisibility(b -> general.getValue()).build());
    ...
    private Setting<Boolean> EntityIgnore = register(Settings.booleanBuilder("Bug1").withValue(false).withVisibility(b -> general.getValue()).build());

    private Setting<Boolean> renderplace = register(Settings.b("Color", false));
    ...
    public void onUpdate() {
        if ((Boolean) backToDefault.getValue()) {
            ...
            AutoCrystalPVPInfo.setValue(false);
        ...
        if (lastTarget instanceof EntityPlayer && ModuleManager.getModuleByName("AutoEZ").isEnabled()) {
            water.godcoder.module.modules.chat.AutoEZ AutoEZ = (AutoEZ) ModuleManager.getModuleByName("AutoEZ");
        ...
        if (render != null) {
            switch (mode.getValue()) {
                case SOLID: {
                    ...
                    KamiTessellator.drawBox(render, red.getValue(), green.getValue(), blue.getValue(), alpha.getValue(), 63);
                ...
                case SOLIDFLAT: {
                    KamiTessellator.prepare(7);
                    KamiTessellator.drawFace(render, red.getValue(), green.getValue(), blue.getValue(), alpha.getValue(), 63);
                    KamiTessellator.release();
                    break;
                }
                case FULL: {
    ...
    protected void onEnable() {
        ...
        if(AutoCrystalPVPInfo.getValue()) {
            ModuleManager.getModuleByName("AutoCrystalPVPInfo").enable();
        }
        if(AutoCrystalPVPInfo.getValue()) {
    ...
    public void onDisable() {
        ...
        if(AutoCrystalPVPInfo.getValue()) {
            ModuleManager.getModuleByName("AutoCrystalPVPInfo").disable();
        }
        if(AutoCrystalPVPInfo.getValue()) {
